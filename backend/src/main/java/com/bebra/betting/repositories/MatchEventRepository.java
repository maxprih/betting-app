...
public interface MatchEventRepository extends JpaRepository<MatchEvent, Integer> {
    ...
    @Query("SELECT new org.bebra.dto.MatchEventDto(me.id, me.name, me.coefficient, m.id, m.firstPlayer.name, m.secondPlayer.name, m.date, me.status) " +
    ...
    @Query("select new org.bebra.dto.MatchEventDto(me.id, me.name, me.coefficient, m.id, m.firstPlayer.name, m.secondPlayer.name, m.date, me.status) FROM MatchEventBet meb JOIN MatchEvent me on meb.matchEvent.id = me.id JOIN Match m  on me.match.id = m.id where meb.bet.id = :betId")
